      ...
      <section className="padding-vertical-xxlg">
        <div className="container row">
          <div id="top" className="col">
            ...
            <p>When enabled Double Call mode will audibly call the number a second time. <em>For example:</em> the caller will read out 26 followed by "2 6" with each number separated phonetically. This is meant to emulate how callers call the numbers in bingo halls.</p>

            <section className="margin-top-xxlg padding-top-sm pale-gray-bg"></section>

            <h4>Chatty Mode</h4>
            <p>When enabled Chatty mode will enable additional chatter from the audible caller. <em>For example:</em> at the start of every game the caller will say "Let's Play Bingo!" or if using the Wild Bingo game mode, "Let's Play Wild Bingo!". It will also call out the fun names for each number like when "1" is called it might say "Number Ace".</p>
            ...
            <h3>Wild Bingo</h3>
            <p>Wild Bingo indicates that the first number called is wild, meaning you would mark every number ending in the same digit as the wild bingo number. <em>For Example:</em> If the wild number is G56, you would mark every number ending in 6.</p>
            ...
            <p className="margin-top-none">Double check your settings to ensure that you're not inadvertently disabling functionality. <em>For example</em>, if you enable manual calling mode, all buttons will be disabled.</p>
